...
export async function apiRequest(endpoint, options = {}) {
  const response = await fetch(`${API_BASE}${endpoint}`, {
    ...
    // Send/receive the httpOnly JWT cookie the backend sets on login.
    // Required for cross-origin requests (e.g. the Codespaces setup) even
    // though the cookie itself isn't readable from JS.
    credentials: 'include',
    ...options
  });
